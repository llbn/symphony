defmodule Symphony.Config do
  ...
  @default_active_states ["opened"]
  @default_terminal_states ["closed"]
  @default_gitlab_endpoint "https://gitlab.com/api/v4"
  ...
  @workflow_options_schema NimbleOptions.new!(
                             tracker: [
                               ...
                               keys: [
                                 ...
                                 endpoint: [type: :string, default: @default_gitlab_endpoint],
  ...
  @spec gitlab_endpoint() :: String.t()
  def gitlab_endpoint do
    validated_workflow_options()
    |> get_in([:tracker, :endpoint])
    |> resolve_env_value(resolve_first_non_empty_env(["GITLAB_URL", "SYMPHONY_GITLAB_BASE_URL"]))
    |> normalize_gitlab_endpoint()
  end

  @spec gitlab_api_token() :: String.t() | nil
  ...
  @spec gitlab_project_id() :: String.t() | nil
  def gitlab_project_id do
    validated_workflow_options()
    |> get_in([:tracker, :project_id])
    |> normalize_secret_value()
    |> case do
      nil ->
        validated_workflow_options()
        ...
        |> normalize_secret_value()
        |> normalize_gitlab_project_identifier()
  ...
  @spec gitlab_project_slug() :: String.t() | nil
  def gitlab_project_slug do
    gitlab_project_id()
  end

  @spec gitlab_assignee() :: String.t() | nil
  def gitlab_assignee do
    validated_workflow_options()
    |> get_in([:tracker, :assignee])
    |> resolve_env_value(resolve_first_non_empty_env(["GITLAB_ASSIGNEE"]))
    |> normalize_secret_value()
  end

  @spec gitlab_active_states() :: [String.t()]
  def gitlab_active_states do
    get_in(validated_workflow_options(), [:tracker, :active_states])
  end

  @spec gitlab_terminal_states() :: [String.t()]
  def gitlab_terminal_states do
  ...
  def validate! do
    with {:ok, _workflow} <- current_workflow(),
         ...
         :ok <- require_gitlab_token(),
         :ok <- require_gitlab_project(),
  ...
  defp require_gitlab_token do
    case tracker_kind() do
      "gitlab" ->
        if is_binary(gitlab_api_token()) do
          :ok
        else
  ...
  defp require_gitlab_project do
    case tracker_kind() do
      "gitlab" ->
        if is_binary(gitlab_project_id()) do
          :ok
        else
  ...
  defp normalize_gitlab_endpoint(endpoint) when is_binary(endpoint) do
    trimmed = String.trim(endpoint)

    cond do
      trimmed == "" ->
        @default_gitlab_endpoint

      String.match?(trimmed, ~r{\/api\/v4/?$}) ->
        String.trim_trailing(trimmed, "/")

      true ->
        String.trim_trailing(trimmed, "/") <> "/api/v4"
    end
  end

  defp normalize_gitlab_endpoint(_endpoint), do: @default_gitlab_endpoint

  defp normalize_gitlab_project_identifier(nil), do: nil

  defp normalize_gitlab_project_identifier(value) when is_binary(value) do
    trimmed = String.trim(value)
  ...
  defp normalize_gitlab_project_identifier(_value), do: nil
  ...
  defp resolve_first_non_empty_env(env_names) when is_list(env_names) do
    Enum.find_value(env_names, fn
