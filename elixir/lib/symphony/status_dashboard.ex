defmodule Symphony.StatusDashboard do
  ...
  defp refresh_runtime_config(%__MODULE__{} = state) do
    %{
      ...
      | enabled:
        ...
        render_interval_ms:
          state.render_interval_ms_override || Config.observability_render_interval_ms()
    }
  end
    ...
    project_part =
      case Config.gitlab_project_id() do
        project_id when is_binary(project_id) and project_id != "" ->
          colorize(gitlab_project_url(project_id), @ansi_cyan)
  ...
  defp gitlab_project_url(project_id) do
    "#{Config.gitlab_endpoint()}/projects/#{URI.encode_www_form(project_id)}/issues"
  end
