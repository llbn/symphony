defmodule Symphony.Orchestrator do
  @moduledoc """
  Polls GitLab and dispatches repository copies to Codex-backed workers.
  """
  ...
  defp maybe_dispatch(%State{} = state) do
    ...
    with :ok <- Config.validate!(),
      ...
      {:error, reason} ->
        Logger.error("Failed to fetch from GitLab: #{inspect(reason)}")
        state
  ...
  defp run_terminal_workspace_cleanup do
    case Tracker.fetch_issues_by_states(Config.gitlab_terminal_states()) do
      {:ok, issues} ->
        issues
        |> Enum.each(fn
