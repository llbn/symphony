defmodule Symphony.ExtensionsTest do
  ...
  setup do
    gitlab_client_module = Application.get_env(:symphony, :gitlab_client_module)

    on_exit(fn ->
      if is_nil(gitlab_client_module) do
        Application.delete_env(:symphony, :gitlab_client_module)
      else
        Application.put_env(:symphony, :gitlab_client_module, gitlab_client_module)
      end
    end)
  ...
  test "tracker delegates to memory and gitlab adapters" do
  ...
  test "gitlab adapter delegates reads and validates mutation responses" do
    Application.put_env(:symphony, :gitlab_client_module, FakeGitLabClient)
    ...
    assert_receive {:create_comment_called, "issue-1", "hello"}

    Process.put({FakeGitLabClient, :create_comment_result}, {:error, :boom})
    assert {:error, :boom} = Adapter.create_comment("issue-1", "boom")
    Process.delete({FakeGitLabClient, :create_comment_result})

    assert :ok = Adapter.update_issue_state("issue-1", "Done")
    assert_receive {:update_issue_state_called, "issue-1", "Done"}
    ...
    Process.delete({FakeGitLabClient, :update_issue_state_result})
  end
  ...
  test "http server serves html and json endpoints end-to-end" do
    ...
    assert %{
             ...
             "running" => [
               %{"issue_identifier" => "MT-HTTP", "last_message" => "rendered", "turn_count" => 7}
             ],
             "retrying" => [%{"issue_identifier" => "MT-RETRY", "error" => "boom"}]
           } = Jason.decode!(body)

    :sys.replace_state(orchestrator_pid, fn state ->
      update_in(state.running["issue-http"].last_codex_message, fn _ ->
        %{message: "structured"}
      end)
    end)
    ...
    assert %{"coalesced" => false, "operations" => ["poll", "reconcile"], "queued" => true} =
             Jason.decode!(body)
  end
  ...
  test "http server child spec, ignore branch, invalid host, and bound_port fallback behave as expected" do
    ...
    Application.put_env(:symphony, :server_port_override, 0)

    {:ok, default_pid} = HttpServer.start_link()
    on_exit(fn -> if Process.alive?(default_pid), do: Process.exit(default_pid, :normal) end)

    {:ok, localhost_pid} =
      HttpServer.start_link(name: :localhost_server, host: "localhost", port: 0)
    ...
    assert {:ok, {0, 0, 0, 0, 0, 0, 0, 1}} =
             HttpServer.parse_host_for_test({0, 0, 0, 0, 0, 0, 0, 1})
  ...
  test "http server covers callback branches and synthetic snapshot payloads" do
    ...
    assert {:error, :bad_request} =
             HttpServer.parse_raw_request_for_test("GET /api/v1/state HTTP/1.1")
    ...
    on_exit(fn ->
      ...
      if Process.alive?(unexpected_orchestrator_pid),
        do: Process.exit(unexpected_orchestrator_pid, :normal)
    end)
    ...
    {:ok, client_socket} =
      :gen_tcp.connect(~c"127.0.0.1", listen_port, [:binary, {:active, false}], 1_000)
  ...
  defp build_http_request(method, path, body, extra_headers) do
    ...
    headers =
      if is_binary(body) and
           not Enum.any?(headers, fn {name, _value} -> name == "content-length" end) do
